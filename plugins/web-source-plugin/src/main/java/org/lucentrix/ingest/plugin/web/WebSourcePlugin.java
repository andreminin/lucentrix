package org.lucentrix.ingest.plugin.web;
...
public class WebSourcePlugin extends AbstractPlugin<WebSourceConfig, SourceContext> implements SourcePlugin {
    ...
    public WebSourcePlugin(WebSourceConfig config, SourceContext context) {
    ...
    public ChangePage next() {
        ...
        while (!crawlPointUrls.isEmpty() && events.size() < context.getPageSize()) {
            ...
            try {
                ContentChange event = crawlPage(url);
                events.add(event);
                visited.add(url);
    ...
    private ContentChange crawlPage(String url) throws IOException {
