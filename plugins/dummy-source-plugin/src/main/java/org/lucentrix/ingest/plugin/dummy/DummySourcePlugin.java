package org.lucentrix.ingest.plugin.dummy;
...
import org.lucentrix.ingest.plugin.dummy.model.insurance.*;
...
public class DummySourcePlugin extends AbstractPlugin<DummySourceConfig, SourceContext> implements SourcePlugin {
    ...
    public DummySourcePlugin(DummySourceConfig config, SourceContext context) {
    ...
    @Override
    public ChangePage next() {
        if (!hasNext()) {
            return ChangePage.builder().cursor(cursor).items(Collections.emptyList()).build();
        }

        List<ContentChange> events = generator.next(context.getPageSize());
        ...
        return ChangePage.builder().items(events).cursor(cursor).hasNext(hasNext()).build();
    }
